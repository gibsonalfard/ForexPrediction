...
# Get New Data
newData = [108.79, 108.79, 108.75, 108.75]
...
for i in range(0, training_set_.shape[0]-1):
    if (training_set_[i, 5] == 1):
        signalValue[0][4] += 1;
        checker(training_set_[i, :],newData,signalValue[0])
    elif (training_set_[i, 5] == -1):
        signalValue[1][4] += 1;
        checker(training_set_[i, :],newData,signalValue[1])
    else:
        signalValue[2][4] += 1;
        checker(training_set_[i, :],newData,signalValue[2])
