...
for col in range(2,3):
    ...
    training_set_ = dataset_train_.iloc[:, col:col+1].values
    ...
    real_stock_price = dataset_test.iloc[:, col:col+1].values
    ...
    dataset_total = pd.concat((dataset_train_[get], dataset_test[get]), axis = 0)
    ...
    plt.title('Forex '+ get+ ' Price Prediction')
    plt.xlabel('Time')
    plt.ylabel('Forex Price')
    plt.legend()
    plt.show()

with open('../Resources/EURJPY_new_train.csv', 'a') as f:
